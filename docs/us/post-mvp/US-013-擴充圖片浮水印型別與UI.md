...
**驗收條件**：
- [ ] `CustomImageBlock` 新增 `watermark?: { enabled: boolean; text: string }` 欄位
- [ ] 圖片區塊被選取時，出現浮水印開關（如按鈕或 toggle），預設為「關閉」
- [ ] 啟用浮水印時，圖片區塊上立即顯示浮水印文字，位置在圖片底部（距離底部約 10% 區塊高度）
- [ ] 浮水印文字為半透明（如 rgba(255,0,0,0.5)）、適當字級（依區塊高度等比縮放），垂直置中於底部區域
- [ ] 關閉浮水印時，浮水印文字立即消失
- [ ] 切換頁面、縮放、拖曳圖片後，浮水印仍正確顯示
- [ ] 未選取圖片區塊時，浮水印仍維持顯示（不因失焦而消失）
- [ ] `npx tsc --noEmit` 無錯誤
