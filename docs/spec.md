...
### 3.2 PDF 填寫 (`/fill/[id]`)
- **PDF 預覽**（react-pdf 渲染，置中於畫面，最大寬度 1080px）
  - 分頁導覽（上一頁/下一頁/跳頁）
  - 縮放控制
- **自訂區塊工具** — 在 PDF 上疊加文字或圖片區塊
  - 「新增文字」按鈕 → 在頁面中心建立可拖曳、縮放、編輯的文字區塊
  - 「新增圖片」按鈕 → 上傳 PNG/JPG 圖片置入頁面，可拖曳、等比例縮放
- **簽名工具**
  - 繪製簽名（可調整畫筆粗細、清除重畫）
  - 可儲存至素材庫
  - 可從素材庫選取既有簽名
- **匯出按鈕**
  - 將目前的文字區塊、圖片區塊及簽名寫入 PDF 並觸發下載
- **表單欄位面板**（目前隱藏）
  - 原先位於右側的 AcroForm 欄位列表已暫時隱藏
  - 因多數使用情境為無 AcroForm 欄位的 PDF，集中版面給自訂區塊編輯
